...
`MORFEExamples` Jupyter kernel. See the [repository README](../README.md) for
details. The generated `Manifest.toml` stays local and is not committed.

Open and execute [`from_a_mesh_to_a_rom.ipynb`](from_a_mesh_to_a_rom.ipynb) with
that kernel selected. It ships at `order = 9`, the conservative reference: about
35 s of solve, and under a minute end to end. The string `dirichlet = "Dirichlet"`
selects the facet group named `Dirichlet` in the Gmsh mesh and fixes all
displacement components on those facets, producing the two clamped ends.
...
`validate.jl` activates the shared environment itself, so it needs no
`--project`. It compares the fresh `R` against
...
results/
  summary.txt
  data/
    W.jls                       # not tracked: 8.6 MB at order 9
    R.jls                       # not tracked
    R_coefficients.csv
