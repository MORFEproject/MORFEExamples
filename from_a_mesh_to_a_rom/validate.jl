...
isfile(fresh) ||
    error("No fresh results at $fresh. Execute from_a_mesh_to_a_rom.ipynb first.")
...
println(report)
pass || error("The clamped beam deviates from its reference (max rel dev $dev).")
...
@assert isfile(backbone) "No backbone at $backbone. Execute from_a_mesh_to_a_rom.ipynb first."
...
println("Clamped-beam validation passed$(FAST ? " (FAST profile)" : "").")
...
#   git add reference_data/ && git commit -m "Bless the clamped-beam reference"
