...
Both files are written by `from_a_mesh_to_a_rom.ipynb`, which ships at `order = 9`,
so the committed notebook *is* the reference run. Regenerating means executing it
and copying `results/data/` over this directory:
...
`results/data/backbone.csv` this notebook writes. The copy lives in the MORFE
repository next to the chart generator, so the site builds without this
...
## What is tracked under `results/`

`W.jls` is 8.6 MB at order 9 and `R.jls` is small but derived, so both are
gitignored: large binaries belong in the `MORFE_results_archive` repository, not
here. The text outputs a reader actually wants are tracked, including
`backbone.csv` and `W_probe_coefficients.csv`, which is what lets the tutorial
figure be reproduced without rerunning the solve.
