...
The outer-mode promotion study (mode promotion, near-resonance tables, fold
analysis, Padé-resummed branches and the DNS reference) is a research example rather
than a tutorial, and lives in the MORFEFerrite working tree rather than here.
...
That installs MORFE, MORFEFerrite and the plotting stack, and registers the
`MORFEExamples` Jupyter kernel. See the [repository README](../README.md) for details.
CairoMakie comes with it: the notebook plots the branch itself, since MORFE returns
data and draws nothing.

Open and execute [`karman_vortex_street.ipynb`](karman_vortex_street.ipynb) with that
kernel selected. It ships at order 3 so that the demonstration remains quick, and with
its outputs cleared; the mesh is committed too, so nothing has to be generated first.

From a shell:

```bash
cd karman_vortex_street
python3 -m nbconvert --execute --to notebook --inplace karman_vortex_street.ipynb
julia validate.jl
```
...
results/
  summary.txt
  data/
    W.jls                # not tracked: ~193 MB at order 9
    R.jls                # not tracked
    R_coefficients.csv
    branch.csv
  figures/
