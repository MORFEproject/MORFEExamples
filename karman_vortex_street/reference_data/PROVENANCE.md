...
Set `order = 9` in `karman_vortex_street.ipynb`, execute the notebook, then from this example's
directory:
...
order-3 notebook validates against this order-9 file. Restore `order = 3` before
committing, so the shipped notebook stays the cheap one, and clear its outputs.

## The tutorial's branch data

The Kármán tutorial page plots `branch.v1.csv`, a copy of the order-9
`results/data/branch.csv` this notebook writes. The copy lives in the MORFE repository
next to the chart generator, so the site builds without this repository being checked
out:
