...
#   Set `order = 3` or `order = 9` in karman_vortex_street.ipynb, then execute it.
#   python3 -m nbconvert --execute --to notebook --inplace karman_vortex_street.ipynb
#   julia validate.jl
# The script activates the repository's shared environment itself, so it needs no
# --project; run `julia setup.jl` from the repository root once beforehand.
using Pkg: Pkg
...
isfile(fresh) || error("No fresh results at $fresh. Execute karman_vortex_street.ipynb first.")
...
isempty(fails) ||
    error("The Kármán example deviates from its reference in: $(join(fails, ", ")).")
...
println("\nKármán validation passed.")

# To bless a new reference after a deliberate, reviewed change:
#   Set order = 9 in karman_vortex_street.ipynb and execute it, then
#   NSE.write_invariants(reference, NSE.rom_invariants(NSE.load_rom_poly(fresh)))
#   git add reference_data/ && git commit -m "Bless the Kármán reference"
